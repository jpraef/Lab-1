...
get.dailyUSGS <- function(site, sensor, start, end){
  ...
  print(site)
  print(sensor)
  data <- read.table(paste("http://waterdata.usgs.gov/nwis/dv?cb_",sensor,"=on&format=rdb&site_no=",site,"&referred_mo dule=sw&period=&begin_date=",start,"&end_date=",end, sep = ""), header = T) #header = T removes one of the two unnecessary rows
...
```{r Step 2}
get.dailyUSGS("11335000", "00060", "10/02/1907", "09/30/2015")
...
#removing leap days with the filter function in dplyr
water_dat <- filter(water_dat, month  != 2 & day !=29) 

#The function ts will create a time series object, ts, with attributes that make time series analysis easier, such as aligning time axes. Yearly totals will have seasonal variation, so the following functions aggregate by month


water_dat_mon_sum <- ts(aggregate(discharge_afd ~ water_month + water_year, data = water_dat, FUN = sum), frequency = 12)

water_dat_mon_mean <- ts(aggregate(discharge_afd ~ water_month + water_year, data = water_dat, FUN = mean), frequency = 12)

water_dat_mon_max <- ts(aggregate(discharge_afd ~ water_month + water_year, data = water_dat, FUN = max), frequency = 12)
```
Results:  
Discussion:  
Limitation:  
