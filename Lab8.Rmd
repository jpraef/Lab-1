...
sapply(c("akima", "ggplot2", "reshape2", "raster","gridExtra", "maptools"), ndpkg)
...
p1 <- ggplot(ch_16m, aes(x = x, y = y, z = Chlorophyll, fill = Chlorophyll)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="Spectral", direction = 1, na.value="white")+ theme_minimal() 
p2 <- ggplot(t_16m, aes(x = x, y = y, z = Temperature, fill = Temperature)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="Oranges", direction = 1, na.value="white")+ theme_minimal() 
p3 <- ggplot(do_16m, aes(x = x, y = y, z = Dissolved_O2, fill = Dissolved_O2)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="BuPu", direction = 1, na.value="white")+ theme_minimal() 


#Image and contour, 2/17/05
p4 <- ggplot(ch_17m, aes(x = x, y = y, z = Chlorophyll, fill = Chlorophyll)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="Spectral", direction = 1, na.value="white")+ theme_minimal() 
p5 <- ggplot(t_17m, aes(x = x, y = y, z = Temperature, fill = Temperature)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="Oranges", direction = 1, na.value="white")+ theme_minimal() 
p6 <- ggplot(do_17m, aes(x = x, y = y, z = Dissolved_O2, fill = Dissolved_O2)) + geom_raster() + stat_contour(color = "black", alpha = 1/4) + scale_fill_distiller(palette="BuPu", direction = 1, na.value="white")+ theme_minimal() 



persp(r_21605, theta = 30, phi = 45, col = "lightblue", zlab = "Chlorophyll", xlab = "Longitude", ylab = "Latitude")
