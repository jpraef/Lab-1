...
A spatial dataset of the dead zone in the Triangle Floodplain in the Cosumnes River Preserve was 
Data:  
Code:
```{r Step 1}
...
r_21605_m <- data.frame(Elevation = rev(melt(r_21605$z)$value), x = rep(r_21605$x, each = 40), y = rep(r_21605$y, 40)) #reversing the order of the z term did it.
...
p2 <- ggplot(t_16m, aes(x = y, y = z, z = Temperature, fill = Temperature)) +
  geom_raster() +
  stat_contour(color = "black", alpha = 1/4) +
...
Results:  
Between February 16th and 17th, 2007, the elevation of the floodplain changes little, with the exception of a deepened channel along the southwest edge.  Ahearn et al. 2006 noted that the floodplain was converted from agricultural land, with two ponds added for habitat.  The y shaped shallow area observed in the elevation plot corresponds to the larger of the two ponds, and the smaller shallow area in the northeast corner is the other. The floodplain varies approximately 1.5 m between the lowest point (the southwest corner of the y pond) and the highest point (the northwest corner of the levee).  On 2/17/07, the deepest point of the y-pond extended north and east along the western levee and the deep point dropped approximately .1 m in elevation. 

Chlorophyll is consistent across the 2/16 y pond (~10 μg L−1), with the exception of a spike of 15 μg L−1 on the east side of the pond.  The spike in chlorophyll becomes a sink on 2/17 as areas around the y pond are innundated with chlorophyll laden flooding.

Discussion:  
Limitations:

Citations:
Ahearn, Dylan S., et al. "Priming the productivity pump: flood pulse driven trends in suspended algal biomass distribution across a restored floodplain." Freshwater Biology 51.8 (2006): 1417-1433.